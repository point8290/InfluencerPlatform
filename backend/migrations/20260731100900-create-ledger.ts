 ...
 *
 * Singular `ledger`, not `ledger_entries`: it is one conceptual object, and it
 * should read that way in the sentence "the ledger is authoritative".
 *
 * There is no updated_at. Append-only means rows are never modified, and a
 * column implying otherwise would be a lie in the schema.
 *
 ...
 * the reverse. Two nullable foreign keys were chosen over a polymorphic
 * reference_type/reference_id pair precisely because a real FK and a real
 * per-type UNIQUE can hang off each — the polymorphic form can give neither.
 */
