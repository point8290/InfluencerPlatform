...
- **A payment with no checkout URL is resumed, not refused.** The Stripe call itself carries an idempotency key derived from `payments.id`, with every parameter read from the frozen row. So a replay that finds the URL still `NULL` — because the first request's Stripe call failed, lost its response, or is still running — simply calls Stripe again: a finished first call yields the *same* session, a running one yields Stripe's in-progress conflict (`409 IDEMPOTENT_REQUEST_IN_PROGRESS`), and a failed one is completed now. Without this, one Stripe blip wedged the key permanently, because the client correctly retries with the same key. A resume is allowed only for a `pending` row inside Stripe's 24h key retention (23h, with margin); outside that a second payable session could be opened, so the caller gets `409 CHECKOUT_NOT_RESUMABLE` and starts a new purchase.
...
| Session creation fails (after insert)                  | `pending` row with no session. Harmless. A retry with the same `Idempotency-Key` resumes it through Stripe's own idempotency key; without one it expires in place. |
