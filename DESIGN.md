...
- **INR in Stripe test mode — verified, no longer a risk.** This was flagged early because Indian Stripe accounts carry export and currency restrictions that could have invalidated the pricing model. Checked before any payment code was written: the test account creates INR Checkout Sessions (`currency: inr`, `amount_total: 30000`) and a confirmed INR PaymentIntent settles (`status: succeeded`, `amount_received: 30000`). The account's balance currency is USD rather than INR, so it is not an Indian account and those restrictions never applied — INR is the presentment currency and Stripe converts on settlement. Integer paise map directly onto Stripe's smallest-currency-unit convention, so no conversion layer is needed anywhere.
